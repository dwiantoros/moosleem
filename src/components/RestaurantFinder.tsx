  ...
  return (
    <div className="glass-panel rounded-[1.75rem] p-6">
        ...
        <div className="mt-3 rounded-xl border border-slate-200/70 bg-white/70 px-3 py-2 dark:border-slate-700 dark:bg-slate-800/55">
          <div className="mt-1 flex items-center justify-between gap-3">
            ...
            <button
              ...
              className="rounded-full bg-teal-100 px-3 py-2 cursor-pointer transition hover:bg-teal-200 dark:bg-teal-900/40 dark:hover:bg-teal-900/60"
              title="Klik untuk melihat logo lebih besar"
            >
              {activeLogo.mark}
            </button>
          </div>
      ...
      {logoModalOpen && (
        ...
        >
          <div
            className="relative w-full max-w-sm rounded-3xl bg-white/95 p-8 shadow-2xl dark:bg-slate-800/95 backdrop-blur-sm"
            onClick={(e) => e.stopPropagation()}
          >
            <button
              onClick={() => setLogoModalOpen(false)}
              className="absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 text-slate-600 transition hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600"
            >
            ...
            <div className="flex flex-col items-center gap-6 text-center">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-600 dark:text-slate-400">Logo Halal</p>
                <p className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">{activeLogo.label}</p>
              </div>
              <div className="flex h-32 w-32 items-center justify-center rounded-2xl border-4 border-slate-200 bg-white/60 dark:border-slate-600 dark:bg-slate-700/40">
                <div className="scale-[3]">{activeLogo.mark}</div>
              </div>
              <div className="space-y-3 text-left">
                <div>
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Bahasa</p>
                  <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">{activeLogo.languageTag}</p>
                </div>
                <div>
                  <p className="text-xs font-semibold text-slate-500 dark:text-slate-400">Keterangan</p>
                  <p className="mt-1 text-sm text-slate-700 dark:text-slate-300">{activeLogo.note}</p>
                </div>
              </div>
            </div>
          </div>
